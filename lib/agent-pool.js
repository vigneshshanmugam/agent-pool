...
module.exports = class AgentPool {
  ...
  fillAndRecycle() {
    ...
    if (this.agents.length >= this.maxAgents) {
      ...
      oldAgent.destroy();
      this.logger.debug(
        `Destroying the old ${this.activeAgent.protocol} agent, Stats - `,
        oldAgent.getCurrentStatus()
      );
    } else if (
      this.activeAgent &&
      ...
      if (socketMetas.length > 0) {
        ...
        for (const meta of socketMetas) {
           ...
           * background host with the acquired meta information
       ...
       */
      setTimeout(
        () => this.changeActiveAgent(newAgent),
        Number(this.destroyTime / 3)
      );
    } else {
      this.logger.debug("Creating agent without preconnected sockets");
      console.log("Creating agent without preconnected sockets");
      const newAgent = this.createAgent();
      this.changeActiveAgent(newAgent);
    }
  }
  ...
  stop() {
    this.recycleTimer && clearTimeout(this.recycleTimer);
  }
